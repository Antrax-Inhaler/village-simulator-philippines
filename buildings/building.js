/* ═══════════════════════════════════════════════════════════════
   ...
   KEY CHANGES
   ─────────────────────────────────────────────────────────────
   NEW: minalangis (Minahan ng Langis)
     — Hall Lv2 required.  prodRes:'langis', prodRate:6.0/level.
     — ONLY building that produces langis.
     — Fully wired for export/import via trade.js.

   REMOVED langis production from:
     paaralan (was 3.0/s) → 0
     templo   (was 5.0/s) → 0
     hukuman  (was 4.0/s) → 0
   Passive langis drip in main.js also removed.
   Storage bonuses (bodega, cuartel, mainHall) unchanged.
═══════════════════════════════════════════════════════════════ */
...
export var BUILDING_DEFS = {
  ...
  /* ── NEW — Oil mine: the ONLY langis producer ────────────── */
  minalangis: {
  ...
  paaralan: {
    ...
    prodRes:null, prodRate:0,                /* was langis 3.0 */
  ...
  templo: {
    ...
    prodRes:null, prodRate:0,                /* was langis 5.0 */
    storageBonus:{ gold:0, rice:0, langis:0 },
  ...
  hukuman: {
    ...
    prodRes:null, prodRate:0,                /* was langis 4.0 */
...
Building.prototype.update = function(dt, VS) {
  ...
  if (def.prodRes && stats.productionRate > 0) {
    this.prodAccum += dt * stats.productionRate * stats.efficiency;
    if (this.prodAccum >= 1) {
      ...
      if (def.prodRes === 'gold') {
        this.uncollectedGold = Math.min(999, this.uncollectedGold + producedZ);
      } else if (def.prodRes === 'rice' || this.type === 'farm') {
        this.uncollectedFood = Math.min(999, (this.uncollectedFood || 0) + producedZ);
      } else if (def.prodRes === 'langis') {
        /* Langis goes directly to VS.res — tap-to-collect like gold would be awkward
           since it's a fuel resource, not a coin. Auto-collect with overflow tracking. */
        VS.res.langis = Math.min(VS.resCap.langis || 9999, (VS.res.langis || 0) + producedZ);
      } else {
...
Building.prototype.draw = function(ctx, now) {
  ...
  /* Inline renderers passed to sprite system as fallback layer */
  ...
  /* drawBuilding checks: sprite file → inline renderer → _drawStandard fallback */
...
/* ══════════════════════════════════════════════════════════════
   DRAW FUNCTIONS — all original + new minalangis
══════════════════════════════════════════════════════════════ */
